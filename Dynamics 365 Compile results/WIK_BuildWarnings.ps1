...
$outputFile = "C:\Temp\" + $packageName + "-Warning.csv"
...
foreach ($result in $results)
{
    try
    {
        $errorText = Select-String -LiteralPath $result.FullName -Pattern '(^.*) Warning: (.*)' | ForEach-Object {$_.Line}
        ...
        if ($errorText)
        {
            ...
            foreach ($line in $errorText)
            {
                ...
                try
                {
                    ...
                    $regex = '(?:Compile Fatal|MetadataProvider|Metadata|Compile|Unspecified|Generation|ExternalReference|BestPractices) (Warning): (Query Method|Interface Method|Form Method LocalFunction|Form Control Method|Form Datasource Method|Form DataSource Method|Form DataSource DataField Method|Form Method|Map Method|Class Delegate|Table Method LocalFunction|Class Method LocalFunction|Table Method|Class Method|Table|Class|View|Form|)(?: |)(?:dynamics:|)(.*)(?:: )(.*)'
