...
$outputFile = "C:\Temp\" + $packageName + "-Task.csv"
...
foreach ($result in $results)
{
    try
    {
        $errorText = Select-String -LiteralPath $result.FullName -Pattern '(^.*)TaskListItem Information: (.*)' | ForEach-Object {$_.Line}
        ...
        if ($errorText)
        {
            ...
            foreach ($line in $errorText)
            {
                ...
                # Remove TODO part
                if ($line -match '(?:TODO :|TODO:|TODO)')
                {
                    $lineReplaced = [regex]::Split($line, '(.*)(?:TODO :|TODO:|TODO:)(.*)')
                    $line = $lineReplaced[1] + $lineReplaced[2]
                }
                ...
                try
                {
                    ...
                    $regex = '(TaskListItem Information): (Query Method|Interface Method|Form Method LocalFunction|Form Control Method|Form Datasource Method|Form DataSource Method|Form DataSource DataField Method|Form Method|Map Method|Class Delegate|Table Method LocalFunction|Class Method LocalFunction|Table Method|Class Method|Table|Class|View|Form|)(?: |)(?:dynamics:|)(.*)(?:: )(.*)'
